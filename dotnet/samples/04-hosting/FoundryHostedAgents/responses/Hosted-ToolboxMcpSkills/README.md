...
A hosted agent that discovers MCP-based skills attached to a Foundry Toolbox and serves them at runtime. Requires TOOLBOX_NAME set to a toolbox with MCP skills. NOTE: this sample references Microsoft.Agents.AI.Mcp, which is not part of the contributor local-feed closure, so the contributor local-framework flow does not apply here.
