...
A translation-chain workflow hosted as a Foundry Hosted Agent using the Responses protocol. Three translation agents are composed as sequential workflow executors (French then Spanish then English) via WorkflowBuilder, then hosted with AddFoundryResponses / MapFoundryResponses.
...
| File | Purpose |
|------|---------|
| `Program.cs` | The agent: builds three translation agents, composes them into a sequential workflow, hosts it with the Responses protocol. |
...
| `HostedWorkflowSimple.csproj` | Self-contained project: single target framework and explicit package versions. It also opts out of the repository's central package management, which does not travel inside the ZIP. |
...
$env:AZURE_AI_AGENT_NAME = "hosted-workflow-simple"
dotnet run -- --local
```
...
```bash
cd dotnet/samples/04-hosting/FoundryHostedAgents/responses/Using-Samples/SimpleAgent
export AZURE_AI_AGENT_NAME="hosted-workflow-simple"
dotnet run -- --local
```

Try: `Translate this text: The weather is beautiful today.`
...
```powershell
$work = Join-Path $env:TEMP "hosted-workflow-simple-work"
mkdir $work
cd $work
```
...
`azd ai agent init` copies the sample into a subfolder named `hosted-workflow-simple` (the top-level `name:`
...
```powershell
$sample = "<repo>/dotnet/samples/04-hosting/FoundryHostedAgents/responses/Hosted-Workflow-Simple/azure.yaml"
...
```
cd hosted-workflow-simple
azd provision
azd deploy
azd ai agent invoke "Translate this text: The weather is beautiful today."
```
...
> curl -X DELETE "<project-endpoint>/agents/hosted-workflow-simple?api-version=v1&force=true" \
...
```powershell
cd $work
<repo>/dotnet/samples/04-hosting/FoundryHostedAgents/scripts/Add-LocalFrameworkFeed.ps1 -Path ./hosted-workflow-simple
```
...
```bash
cd "$WORK"
<repo>/dotnet/samples/04-hosting/FoundryHostedAgents/scripts/add-local-framework-feed.sh ./hosted-workflow-simple
```
