...
A retrieval-augmented (RAG) support agent hosted as a Foundry Hosted Agent using the Responses protocol. A TextSearchProvider runs a search against a knowledge base before each model call and injects the results into the context, then the agent is hosted with AddFoundryResponses / MapFoundryResponses.
...
| File | Purpose |
|------|---------|
| `Program.cs` | The agent: wires a TextSearchProvider (mock search) into the agent context, hosts it with the Responses protocol. |
...
| `HostedTextRag.csproj` | Self-contained project: single target framework and explicit package versions. It also opts out of the repository's central package management, which does not travel inside the ZIP. |
...
$env:AZURE_AI_AGENT_NAME = "hosted-text-rag"
dotnet run -- --local
```
...
```bash
cd dotnet/samples/04-hosting/FoundryHostedAgents/responses/Using-Samples/SimpleAgent
export AZURE_AI_AGENT_NAME="hosted-text-rag"
dotnet run -- --local
```
...
```powershell
$work = Join-Path $env:TEMP "hosted-text-rag-work"
mkdir $work
cd $work
```
...
`azd ai agent init` copies the sample into a subfolder named `hosted-text-rag` (the top-level `name:`
...
```powershell
$sample = "<repo>/dotnet/samples/04-hosting/FoundryHostedAgents/responses/Hosted-TextRag/azure.yaml"
...
> curl -X DELETE "<project-endpoint>/agents/hosted-text-rag?api-version=v1&force=true" \
...
```powershell
cd $work
<repo>/dotnet/samples/04-hosting/FoundryHostedAgents/scripts/Add-LocalFrameworkFeed.ps1 -Path ./hosted-text-rag
```
...
```bash
cd "$WORK"
<repo>/dotnet/samples/04-hosting/FoundryHostedAgents/scripts/add-local-framework-feed.sh ./hosted-text-rag
```
