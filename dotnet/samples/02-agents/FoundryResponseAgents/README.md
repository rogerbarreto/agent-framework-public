...
These samples demonstrate how to use the `FoundryAgentClient` (backed by the **Responses API**) to work with
Microsoft Foundry models directly, without creating server-side agent definitions.

## How these differ from [Foundry Agents](../FoundryAgents/README.md)

| | **Foundry Agents** | **Foundry Response Agents** |
|---|---|---|
...
| **Type** | `AIAgent` via `AIProjectClient.CreateAIAgentAsync` | `FoundryAgentClient` (wraps `ChatClientAgent` internally) |
